...
2.2.0 新增 Colossal、Design Milk 和 Are.na；2.3.0 增加 Feature Shoot 与 My Modern Met；2.4.0 增加 Aperture 与 PRINT Magazine。沿用现有工作流，均无需注册、密钥或浏览器扩展。
...
| Aperture | 摄影作品集 | 摄影书、经典档案、摄影师访谈、摄影评论、展览与书评 | 英文关键词，例如 `portrait`；留空浏览分类 |
| PRINT Magazine | 平面设计 | 品牌与标识、字体与排版、插画、包装、海报、设计史 | 英文关键词，例如 `typography`；留空浏览分类 |
...
Colossal、Design Milk、Feature Shoot、My Modern Met、Aperture 和 PRINT Magazine 每张卡片代表一个专题，详情可切换多张图片，并全屏缩放、拖拽查看。“下载图集”保存该专题整组图片，上限60张。只提供单专题下载，不放置容易一次下载数百张的“下载本页”按钮。
...
- 六个专题来源缓存100个新专题的封面，以及说明、图集地址等资料，不下载每个专题全部原图。
- Are.na 缓存100张新图片的预览及说明。
- 已有缓存跳过，进度和索引沿用现有机制，重复点击按当前查询条件从断点继续。
- 搜索再次命中已缓存素材时使用本地预览和详情；高清图集仍按需联网下载。

下载仍跟随 ComfyUI 的输出目录，在 `output/ty-node/ty-image-spider/` 下按来源建立 `colossal/`、`aperture/`、`printmag/` 等子目录。专题图片文件名使用 `专题ID-序号`，Are.na 使用图片块 ID。缓存仍共用 `cache/index.sqlite3`。
...
- `EditorialProvider`：六个 WordPress 公开专题源共用的检索和下载策略；每站差异由不可变配置提供。
...
- [Feature Shoot](https://www.featureshoot.com/)、[My Modern Met](https://mymodernmet.com/)、[Aperture](https://aperture.org/) 与 [PRINT Magazine](https://www.printmag.com/) 的 WordPress 公开专题接口。
...
2.4.0 的 Aperture 与 PRINT 前两页各返回12项且无跨页重复，首个专题分别解析8张和1张图片。两者均真实新增缓存100项：Aperture 无失败；PRINT 有1张来源 PNG 响应截断，任务记录失败后继续补足100项。
