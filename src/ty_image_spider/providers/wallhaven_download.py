...
class WallhavenDownloader:
    ...
    def download(self, url: str, item_id: str, output_root: Path) -> DownloadResult:
        try:
            require_https_host(url, lambda host: host == "w.wallhaven.cc")
        except SpiderError as exc:
            raise SpiderError(
                "unsafe_download_url", "Wallhaven 图片地址不受信任"
            ) from exc
        if not _SAFE_ID.fullmatch(item_id):
            raise SpiderError("invalid_asset", "Wallhaven 素材 ID 无效")

        directory = resolve_inside(output_root, Path("ty-image-spider/wallhaven"))
        ...
        try:
            with self._open_url(request, timeout=60) as response:
                try:
                    require_https_host(
                        response.geturl(), lambda host: host == "w.wallhaven.cc"
                    )
                except SpiderError as exc:
                    raise SpiderError(
                        "unsafe_redirect", "Wallhaven 图片下载发生了不安全的重定向"
                    ) from exc
            ...
            raise SpiderError(
                "download_failed", "Wallhaven 图片下载失败", status=502
            ) from exc
        ...
        try:
            ...
            extension = _verify_image(temp)
            target = _available_target(directory, item_id, extension)
            os.replace(temp, target)
        finally:
            temp.unlink(missing_ok=True)

        relative = target.relative_to(Path(output_root).resolve()).as_posix()
        return DownloadResult((relative,), "Wallhaven 图片已下载")
